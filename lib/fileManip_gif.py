...
class fileManipulation_gif( fileManipulation ):

   extension = ".gif"

   def __init__( self, meta, conf, logName ):
      fileManipulation.__init__( self, conf, logName )
      self.logger = logging.getLogger(logName+".fileManipulation_gif")
      self.logger.info("fileManipulation_gif")
      self.main( meta )
      ...
      for one in meta:
         if "File:FileModifyDate" in one:
            ...
            d = { "unknown" : 0
                 ...
                 ,"newFileName" : one["File:Directory"] 
                } 
            a.append( d )
            un += 1

      self.logger.info( "Found %s valid GIF's" % ( good ))
      self.logger.info( "Found %s unknown GIF's" % ( un ))
      self.yahoo( a )
